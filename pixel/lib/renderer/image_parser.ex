defmodule Pixel.Renderer.ImageParser do
  require Logger
  ...
  defp do_png_chunk("iCCP", data) do
    ...
    <<
      # 0-3
      profile_size::unsigned-size(4)-unit(8),
      # 4-7
      preferred_cmm_type::binary-size(4),
      # profile_version_raw::binary-size(2), # 8-11
      0::size(4),
      profile_version_major::size(4),
      profile_version_minor::size(4),
      0::size(4),
      # Lowest two are reserved
      0::unit(8)-size(2),
      # 12-15
      profile_class::binary-size(4),
      # 16-20
      color_space::binary-size(4),
      # 20-23
      pcs::binary-size(4),
      # 24-35
      creation_date_time::binary-size(12),
      # 36-39
      "acsp",
      # 40-43
      primary_platform_signature::binary-size(4),
      # profile_flags::binary-size(4), # 44-47
      flag_embedded::size(1),
      flag_independent::size(1),
      0::size(30),
      # 48-51
      device_manufacturer::binary-size(4),
      # 52-55
      device_model::binary-size(4),
      ...
      0::size(28),
      # really, 60-63
      device_attributes::binary-size(4),
      # 64-67
      rendering_intent::binary-size(4),
      # 68-79
      nciexyz::binary-size(12),
      # 80-83
      signature::binary-size(4),
      # 84-99
      profile_id::binary-size(16),
      # 100-127
      0::unit(8)-size(28)
    >> = header
  ...
  defp parse_tag_table(tag_table) do
    ...
    table =
      Enum.map(table, fn {sig, offset, size} ->
  ...
  defp do_parse_signature(
         <<"XYZ ", 0::32, x_sign::16, x::signed-16, y_sign::16, y::16, z_sign::16, z::16>>
       ) do
  ...
  defp do_parse_signature(
         <<"view", 0::32, illuminant::binary-size(12), surround::binary-size(12), type::binary>>
       ) do
    {illuminant, surround, type}
  end

  defp do_parse_signature(
         <<"meas", 0::32, observer::32, tristimulus::binary-size(12), geo::32, flare::32,
           illuminant::32>>
       ) do
  ...
  defp do_parse_tag_table(
         <<signature::binary-size(4), offset::32, size::32, next::binary>>,
         acc,
         count
       ) do
